...
class organisme_services
{
    ...
    public static function lister_par_recruteur($idr)
    {
        $resultat = NULL;
        ...
        {
            $pdo = database_lib::generate_pdo_object();
            $stmt = $pdo->prepare('SELECT * FROM `organisme` WHERE `etat_organisme` = 1 AND `recruteur_organisme` = :idr');
    ...
    public static function chercher($info)
    {
        ...
        try 
        {
            ...
            $stmt = $pdo->prepare('SELECT * 
                                     FROM `organisme` 
                                    WHERE `raison_sociale_organisme`    LIKE :info OR
                                          `specialite_organisme`        LIKE :info OR
                                          `fixe_organisme`              LIKE :info OR
                                          `fax_organisme`               LIKE :info OR
                                          `email_organisme`             LIKE :info OR
                                          `adresse_organisme`           LIKE :info OR
                                          `code_postal_organisme`       LIKE :info OR
                                          `nrc_organisme`               LIKE :info OR
                                          `rib_organisme`               LIKE :info OR
                                          `numero_soumission_organisme` LIKE :info');
